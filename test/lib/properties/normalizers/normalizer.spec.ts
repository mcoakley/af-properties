...
class StringNormalizerTester
    extends Normalizer<string, NormalizerOptions>
{
...
describe("Normalizer class", function() {
    ...
    describe("Test the constructor and options", function() {
            ...
            function()
        {
            ...
            expect(new StringNormalizerTester(undefined))
            .to.be.an.instanceof(StringNormalizerTester);

            // @ts-ignore
            expect(new StringNormalizerTester(null))
                .to.be.an.instanceof(StringNormalizerTester);

            expect(new StringNormalizerTester({ name: "" }))
                .to.be.an.instanceof(StringNormalizerTester);

            const gp = new Property<string>(
                STRING_DATA, { name: NORMALIZER_NAME }
            );
            expect(new StringNormalizerTester({ parent: gp }))
                .to.be.an.instanceof(StringNormalizerTester);
        });
    ...
    describe("Testing functionality of the class as a string", function() {
        ...
        it("validates that the object gets a default name", function() {
            ...
            expect(options.name).to.equal(DEFAULT_NORMALIZER_NAME + "_" +
                Normalizer.getNormalizerCount());
        });
    });
});
