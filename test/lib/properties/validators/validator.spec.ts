...
abstract class ValidatorTester<T>
    extends Validator<T, PropertyValidatorTesterOptions>
{
    ...
    protected _validateOptions(newOptions: PropertyValidatorTesterOptions)
        : void {
        super._validateOptions(newOptions);

        this.options.delayResponse = this.options.delayResponse || false;
    }
}
...
class StringValidatorTester extends ValidatorTester<string> {
    protected testValue(value: string): boolean {
        ...
        return isUsable.test(value) && !isEmpty.test(value) &&
            value.length < 80;
    }
}

// This is used for coverage testing
class StringValidatorEnrichmentTester
    extends ValidatorTester<string>
{
    protected testValue(value: string): boolean {
        const enrichedData = {
            firstThree: "Hel"
...
class NumberValidatorTester extends ValidatorTester<number> {
    protected testValue(value: number): boolean {
        return isNumber.test(value) &&
            ((this.options.required && !isNaN(value)) ||
...
describe("Validator class", function () {
    ...
    describe("Test the construction and options", function () {
        it("tests construction of the object with options to test all paths",
            function () {
                expect(new StringValidatorTester({ name: VALIDATOR_NAME }))
                    .to.be.an.instanceof(StringValidatorTester);

                expect(new StringValidatorTester(undefined))
                    .to.be.an.instanceof(StringValidatorTester);

                // @ts-ignore
                expect(new StringValidatorTester(null))
                    .to.be.an.instanceof(StringValidatorTester);

                expect(new StringValidatorTester({ name: "" }))
                    .to.be.an.instanceof(StringValidatorTester);

                const gp = new Property<string>(
                    STRING_DATA, { name: VALIDATOR_NAME }
                );
                expect(new StringValidatorTester({ parent: gp }))
                    .to.be.an.instanceof(StringValidatorTester);

                expect(
                    new StringValidatorTester(
                        { parent: gp, required: false }
                    )
                ).to.be.an.instanceof(StringValidatorTester);
            });
    ...
    describe("Testing functionality of the class as a String", function () {
        ...
        const LONG_STRING_DATA = "123456789(10)123456789" +
        ...
        it(" 2. tests if the data is empty, required is true",
            async function() {
                expect(await svt.validate(EMPTY_STRING_DATA)).to.be.false;
                expect(svt.getErrors()[0].code)
                    .to.be.equal(ValidatorErrorCodes.RequiredAndEmpty);
            });

        it(" 3. tests if the data is empty, required is false",
            async function() {
                expect(await svt.validate(EMPTY_STRING_DATA)).to.be.false;
                expect(svt.getErrors()[0].code)
        ...
        it(" 4. tests if the data is empty, required is false, validation " +
            "is delayed 200ms",
            async function() {
                ...
                async function secondTest() {
                    ...
                    try {
                        ...
                        while (svt.isValidating()) {
                            i++;
                            if (i === 1) {
                                expect(await svt.validate(EMPTY_STRING_DATA))
                                    .to.be.true; // This is definitely wrong
                            }
                        }
        ...
        it(" 5. tests isValueAllowed and reset and addInvalidError for " +
            "coverage.",
            async function() {
                expect(svt.isValueAllowed(LONG_STRING_DATA)).to.be.true;
                expect(await svt.validate(LONG_STRING_DATA)).to.be.false;
                expect(svt.getErrors()[0].code)
                    .to.be.equal(ValidatorErrorCodes.Invalid);

                svt.reset();
                expect(svt.getErrors().length).to.equal(0);
            });

        it(" 6. tests isValueAllowed while the value is required.",
            async function() {
                expect(await svt.validate(LONG_STRING_DATA)).to.be.false;
                expect(svt.getErrors()[0].code)
                    .to.be.equal(ValidatorErrorCodes.RequiredAndInvalid);
            });

        it(" 7. validates that the object gets a default name", function() {
            const options = svt.getOptions();
            expect(options.name).to.equal(DEFAULT_VALIDATOR_NAME + "_" +
                Validator.getValidatorCount());
        });
    });

    describe("Testing validation of the class with data enrichment",
        function() {
            ...
            it("tests that validate emits the first 3 characters of the data",
                async function() {
    ...
    describe("Testing validation of the class as a Number", function() {
        ...
        it("validates NaN, when required", async function() {
            ...
            expect(nvt.getErrors()[0].code)
                .to.equal(ValidatorErrorCodes.RequiredAndEmpty);
        });
