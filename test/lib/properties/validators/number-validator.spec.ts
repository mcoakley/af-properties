...
describe("NumberValidator class", function() {
    ...
    describe("Test the construction and options", function() {
        ...
        // Regression
        it("ensure minValue set with maxValue 0 does not throw an error" +
            "(regression)", function()
        {
            expect(function() {
                new NumberValidator({
        ...
        it("ensure minValue === maxValue does not throw an error (regression)",
            function()
        {
            expect(function() {
                new NumberValidator({
        ...
        it("ensure minValue < Number.MIN_SAFE_INTEGER throws an error",
            function()
        {
            expect(function() {
                new NumberValidator({
        ...
        it("ensure maxValue > Number.MAX_SAFE_INTEGER throws an error",
            function()
        {
            expect(function() {
                new NumberValidator({
        ...
        it("pulls options from the parent", function() {
            const np = new NumberProperty(NUMBER_DATA,
                { name: VALIDATOR_NAME }
            );
            expect(function() {
    ...
    describe("Testing functionality of the class", function() {
        ...
        it("validates the minValue validation", async function() {
            ...
            expect(nv.getErrors()[0].code)
                .to.equal(ValidatorErrorCodes.Invalid);
        });

        it("validates the minValue validation when required",
            async function()
        {
            expect(await nv.validate(SMALL_NUMBER_DATA)).to.be.false;
            expect(nv.getErrors()[0].code)
                .to.equal(ValidatorErrorCodes.RequiredAndInvalid);
        });

        it("validates the minValue validation when minValue is negative",
            async function()
        {
        ...
        it("validates the maxValue validation when required",
            async function()
        {
            expect(await nv.validate(LARGE_NUMBER_DATA)).to.be.false;
            expect(nv.getErrors()[0].code)
        ...
        it("validates the maxValue validation when maxValue is negative",
            async function()
        {
            expect(await nv.validate(NUMBER_DATA)).to.be.false;
            expect(nv.getErrors()[0].code)
