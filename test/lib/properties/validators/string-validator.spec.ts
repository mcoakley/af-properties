...
describe("StringValidator class", function() {
    ...
    describe("Test the construction and options", function() {
        it("tests construction of the object with options to test all paths",
            function()
        {
            expect(new StringValidator({ name: VALIDATOR_NAME }))
                .to.be.an.instanceof(StringValidator);

            expect(new StringValidator(undefined))
                .to.be.an.instanceof(StringValidator);

            // @ts-ignore
            expect(new StringValidator(null))
                .to.be.an.instanceof(StringValidator);

            expect(new StringValidator({ name: "" }))
                .to.be.an.instanceof(StringValidator);

            const gp = new StringProperty(
                STRING_DATA, { name: VALIDATOR_NAME }
            );
            expect(new StringValidator({ parent: gp }))
                .to.be.an.instanceof(StringValidator);
    ...
    describe("Testing functionality of the class", function() {
        ...
        it("tests if the data is greater than the minLength when " +
            "minLength = 5 (true)", async function()
        {
            expect(await sv.validate(STRING_DATA)).to.be.true;
        });

        it("tests if the data is greater than the minLength when " +
            "minLength = 15 (false)", async function()
        {
            expect(await sv.validate(STRING_DATA)).to.be.false;
            expect(sv.getErrors()[0].code)
                .to.be.equal(ValidatorErrorCodes.Invalid);

            expect(await sv.validate(EMPTY_STRING_DATA)).to.be.true;
        });

        it("tests if the data is greater than the minLength when minLength" +
            " = 15 (false), and required", async function()
        {
            expect(await sv.validate(STRING_DATA)).to.be.false;
            expect(sv.getErrors()[0].code)
        ...
        it("tests if the data is greater than the maxLength when " +
            "maxLength = 20 (true)", async function()
        {
            expect(await sv.validate(STRING_DATA)).to.be.true;
        });

        it("tests if the data is greater than the maxLength when " +
            "maxLength = 10 (false)", async function()
        {
            expect(await sv.validate(STRING_DATA)).to.be.false;
            expect(sv.getErrors()[0].code)
                .to.be.equal(ValidatorErrorCodes.Invalid);
