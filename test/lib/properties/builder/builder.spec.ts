...
describe("Test the propertyBuilder factory", function() {
    ...
    it("returned undefined for an invalid property name", function() {
        expect(propertyBuilder(STRING_DATA, INVALID_PROPERTY_NAME))
            .to.be.undefined;

        // @ts-ignore
        expect(propertyBuilder(STRING_DATA, undefined))
            .to.be.undefined;
    });

    it("builds a string property", function() {
        expect(propertyBuilder(STRING_DATA, STRING_PROPERTY_NAME))
            .to.be.an.instanceof(StringProperty);
    });

    it("builds a Email property (a masked property)", function() {
        expect(propertyBuilder(EMAIL_DATA, EMAIL_PROPERTY_NAME))
