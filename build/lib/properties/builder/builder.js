...
function getPropertyDescription(propertyName, aliasMap) {
    if (!af_conditionals_1.isUsable.test(propertyName))
        return undefined;
    propertyName = propertyName.toLocaleLowerCase();
    const propertyType = (af_conditionals_1.isUsable.test(aliasMap) ?
...
function propertyBuilder(value, propertyName, displayName, aliasMap) {
    ...
    if (af_conditionals_1.isUsable.test(propertyDescription)) {
        propertyDescription.options.name = propertyName;
        if (!af_conditionals_1.isUsable.test(propertyDescription.options.mask)) {
